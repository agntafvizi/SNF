...
BASEFLOW <- BASEFLOW %>%
 transmute(DATE = as.Date(BASEFLOW$Date, format = "%m/%d/%y"), streamflow, precipitation)
...
BASEFLOW<- BASEFLOW %>%
  mutate(DATE,streamflow, precipitation, bfs$bt,bfs$qft)
...
hydrograph(streamflow= BASEFLOW[,2], streamflow2=BASEFLOW[,4], 
           timeSeries = BASEFLOW[,1], precip = BASEFLOW[,3], 
           S.units = "m3s", P.units = "mm")
